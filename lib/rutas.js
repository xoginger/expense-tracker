...
function buildSlug({ origin, destination, slug, name }) {
    return storage.slugFromRuta({ origin, destination, slug, name });
}
...
function createRuta(db, body) {
    ...
    const result = db.prepare(`
      INSERT INTO events (name, description, start_date, end_date, status, origin, destination, slug, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(name, notes, start_date, end_date, status, origin, destination, slug, notes);
...
function updateRuta(db, id, body) {
    ...
    const slug = body.slug !== undefined
        ...
        : buildSlug({ origin, destination, name, slug: current.slug });
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(name, notes, start_date, end_date, status, origin, destination, slug, notes, id);
